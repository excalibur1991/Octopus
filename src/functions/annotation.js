  ...
  if (props.isInEdit) {
    props.dispatch({
      ...
      alertSettings: {
        ...
        type: 'error',
        title: 'Error',
        message: 'Please save current change first.',
        showConfirmButton: true,
        confirmText: 'Ok',
      },
    });
...
export const saveAnnotation = async (props) => {
  ...
  const hasAnonymBounds = _rects.find(
    (value) => value.tag.toLocaleLowerCase() == 'anonymization bounty',
  );
  if (hasAnonymBounds) {
    _rects.push({
      type: 'anonymization',
...
export const handleOnMove = (props, position) => {
  ...
  props.setCropPosition({
    x: cropPosX / props.rectScale,
    y: cropPosY / props.rectScale,
  });
  props.setRectScale(position.scale);
};

export const handleCentering = (
  props,
  imageWidth,
  imageHeight,
  cropWidth,
...
export const handleOnClick = (props, position) => {
  ...
  const _rectX = origLocX,
    _rectY = origLocY,
    _rectWidth = rectWidth / props.rectScale,
    _rectHeight = rectHeight / props.rectScale;
  ...
  const blockX =
    props.cropPosition.x +
    Math.floor((origLocX - props.cropPosition.x) / _rectWidth) * _rectWidth;
  const blockY =
    props.cropPosition.y +
    Math.floor((origLocY - props.cropPosition.y) / _rectHeight) * _rectHeight;
  ...
  if (props.isInEdit == false) {
    ...
    props.annoRect.map((data, data_index) => {
      data.rects.map((rect, index) => {
        if (
          intersect(rect.x, rect.y, rect.width, rect.height, _rectX, _rectY)
        ) {
          selected = true;
  ...
  if (props.isEyeDrop) {
    getImageColor(
      props,
      Math.floor(position.locationX * 2.7),
      Math.floor(position.locationY * 2.7),
    );
    return;
  }
  ...
  if (props.curAnnoMode == 'box') {
    props.tempAnnoRect.rects.map((value, index) => {
      if (value.tag == props.curTag) {
        if (
          intersect(value.x, value.y, value.width, value.height, _rectX, _rectY)
        ) {
          //found
          found = true;
        } else {
    ...
    if (hasTag) {
      props.annoDot.map((value, index) => {
        if (value.tag == props.curTag) {
          var foundedItem = value.dots.find((dot) =>
            intersectCircle(dot.x, dot.y, _rectX, _rectY),
          );
          if (foundedItem) {
            var remained_dots = value.dots.filter(
              (dot) => !intersectCircle(dot.x, dot.y, _rectX, _rectY),
            );
            ...
            _rect.push({
              tag: value.tag,
              type: 'dots',
              dots: [...value.dots, {x: _rectX, y: _rectY}],
            });
          }
        } else {
          //org value
...
const isOverlaps = (x, y, width, height, x2, y2, width2, height2) => {
  if (x < x2 + width2 && y < y2 + height2 && x2 < x + width && y2 < y + height)
    return true;
  return false;
};
...
const get_box_size = (curblock, blocks) => {
  ...
  const t_blocks = blocks.filter(
    (block) => block.width == width && block.height == height,
  );
...
export const optimizeBlocks = (blocks) => {
  ...
  blocks.map((item, index) => {
    ...
    group_keys.map((key) => {
      ...
      blocks_per_width.sort(
        (b1, b2) => b2.width * b2.height - b1.width * b1.height,
      );
...
export const getImageColor = (props, x, y) => {
  ...
  context.getImageData(Math.floor(x), Math.floor(y), 1, 1).then((imageData) => {
    if (imageData) {
...
export const drawCanvas = (props, blob) => {
  ...
  image.addEventListener('load', () => {
    ...
    handleCentering(
      ...
      width,
      height,
      props.frameDimension.width,
      props.frameDimension.height,
      1.0,
    );
  });
};
