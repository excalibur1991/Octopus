...
let myWallet1PKey = "84d8bd3e50eddf675f37227e40df9c395f48367548ea3f9ca4d2ff33a473fe16" //dummy
...
export async function getTokenBalance_() {
  ...
  pool.methods.balanceOf(phecor0RinkebyContract).call(function(err, phecorBal) {
...
export async function allowance(
   ...
   */
 async function getApproval(account, tokenAddress, spender, amount, force = false ) {
    const minABI = [
      {
        constant: false,
        inputs: [
...
export async function getMaxAddLiquidity( poolAddress, tokenAddress) {
 ...
 // const balance = await getReserve(poolAddress, tokenAddress)
   ...
   */
  export async function joinswapExternAmountIn( account,poolAddress,tokenIn,tokenAmountIn, minPoolAmountOut ) {
    const pool = new web3.eth.Contract(jsonpoolABI.abi, poolAddress, {
      from: account
    })
    let result = null
